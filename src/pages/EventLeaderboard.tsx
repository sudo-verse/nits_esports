...
// BGMI match keys - declared outside component to avoid dependency array issues
const BGMI_MATCH_KEYS = ["match1", "match2", "match3", "match4", "match5", "match6"] as const;
type BGMI_MatchKey = typeof BGMI_MATCH_KEYS[number];
...
const EventLeaderboard = () => {
  ...
  const [freefireFinalsRows, setFreefireFinalsRows] = useState<Array<{ id: string; team: string; group: string; booyah: number; placement: number; kills: number; total: number }>>([]);
  // Initialize Free Fire finals from semifinal overview rows and sync team names
  useEffect(() => {
    if (gameId !== 'freefire') return;
    ...
    if (finalsTeams.length > 0) {
      setFreefireFinalsRows((prevRows) => {
        ...
        // If we have previously saved finals, preserve their stats but update team names
        if (prevRows.length > 0) {
          return newRows.map((newRow, idx) => {
            ...
            if (prevRow) {
              return {
                ...newRow,
                booyah: prevRow.booyah,
                placement: prevRow.placement,
                kills: prevRow.kills,
                total: prevRow.total,
              };
            }
  ...
  const getTeamName = useCallback((origIdx: number) => {
  ...
  const overallRows = useMemo(() => {
    if (!matchData || !teamNames || teamNames.length === 0) {
      return [];
    }
  ...
  const [nziggerStats, setNziggerStats] = useState<Omit<FinalsRow, 'origIdx' | 'team'>>({ wwcd: 0, placement: 0, kills: 0, total: 0 });
    ...
    const load = async () => {
      ...
      try {
        ...
        if (snapshot.finals && snapshot.finals.length > 0) {
          ...
          const nziggerRow = finalsList.find((r: any) => r.origIdx === -1 || r.team === 'N-ZIGGER');

          setFinalsRows(
            finalsList.filter((r: any) => r.origIdx !== -1 && r.team !== 'N-ZIGGER').slice(0, 16).map((row: any) => {
          ...
          if (nziggerRow) {
            const placement = Number(nziggerRow.placement ?? 0);
            const kills = Number(nziggerRow.kills ?? 0);
            setNziggerStats({
              wwcd: Number(nziggerRow.wwcd ?? 0),
              placement,
              kills,
              total: Number(nziggerRow.total ?? placement + kills),
            });
          }
        } else {
          setFinalsRows([]);
          setNziggerStats({ wwcd: 0, placement: 0, kills: 0, total: 0 });
        }
  ...
  useEffect(() => {
    ...
    const loadFreefire = async () => {
      ...
      try {
        ...
        // Load Free Fire Finals if saved
        const savedFinals = (snapshot as any)?.finals as any[] | undefined;
        if (savedFinals && Array.isArray(savedFinals) && savedFinals.length > 0) {
          setFreefireFinalsRows(
            savedFinals.map((r: any) => {
      ...
      if (gameId === "bgmi") {
        const payload = {
          ...
          finals: [
            ...
            { origIdx: -1, team: 'N-ZIGGER', wwcd: nziggerStats.wwcd, placement: nziggerStats.placement, kills: nziggerStats.kills, total: nziggerStats.total }
          ],
        };
        ...
        // Save current semifinal rows and finals as edited by admin; do not rebuild and wipe stats
        const payload = { groups: freefireGroups, semifinals: semifinalRows, finals: freefireFinalsRows } as any;
        await savePointsSnapshot(eventId, gameId, payload);
      } else if (gameId === 'codm') {
  ...
  const updateFreeFireFinalsCell = (teamId: string, key: "booyah" | "placement" | "kills", value: string) => {
    const numericValue = Number(value) || 0;
              ...
              <Card className="glass-card border-primary/20">
                <CardHeader>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    {canEdit && (gameId === 'bgmi' || gameId === 'freefire') && (
                      <div className="flex items-center gap-3 text-sm">
                        {isSupabaseConfigured() ? (
                          <>
                            <div className="text-muted-foreground">
                  ...
                  <div className="overflow-x-auto">
                    {gameId === 'freefire' && semifinalOverviewRows.every((r) => (r.totalPoints || 0) === 0) ? (
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {gameId === 'bgmi' ? (
                            [...finalsDisplayRows, { rank: 17, origIdx: -1, team: 'N-ZIGGER', ...nziggerStats }].map((row) => (
                                ...
                                <TableCell className="text-right w-24">
                                  {canEdit ? (
                                    <Input
                                      ...
                                      type="number"
                                      value={row.booyah}
                                      onChange={(e) => updateFreeFireFinalsCell(row.id, "booyah", e.target.value)}
                                    />
                                  ) : (
                                    row.booyah
                                  )}
                                ...
                                <TableCell className="text-right w-36">
                                  {canEdit ? (
                                    <Input
                                      ...
                                      type="number"
                                      value={row.placement}
                                      onChange={(e) => updateFreeFireFinalsCell(row.id, "placement", e.target.value)}
                                    />
                                  ) : (
                                    row.placement
                                  )}
                                ...
                                <TableCell className="text-right w-32">
                                  {canEdit ? (
                                    <Input
                                      ...
                                      type="number"
                                      value={row.kills}
                                      onChange={(e) => updateFreeFireFinalsCell(row.id, "kills", e.target.value)}
                                    />
                                  ) : (
                                    row.kills
                                  )}
